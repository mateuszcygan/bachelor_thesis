...
def learn_probabilities(mdp_object, threshold):
    ...
    exe_actions = aux.flatten_list(exe_actions)
    ...
    while True:
        ...
        # Check convergence
        exe_action_counter = 0
        convergence = True
        for init_state in S:

            executed_action = exe_actions[exe_action_counter]
            exe_action_counter += 1

            for foll_state in S:
                convergence = convergence and (abs(approximated_prob[init_state][executed_action][foll_state] - approximated_prob_new[init_state][executed_action][foll_state]) < threshold)
